...
const REF_TYPE_LABELS: Record<
  UnconnectedReference['type'],
...
export function ProjectHealthDashboard({
  ...
  const totalErrors = useMemo(
    () => allValidationItems.filter((i) => i.severity === 'error').length,
    [allValidationItems]
  );
  const totalWarnings = useMemo(
    () => allValidationItems.filter((i) => i.severity === 'warning').length,
    [allValidationItems]
  );
  const totalInfos = useMemo(
    () => allValidationItems.filter((i) => i.severity === 'info').length,
    [allValidationItems]
  );
  const projectScore = useMemo(
    () => calculateQualityScore(allValidationItems),
    [allValidationItems]
  );
              ...
              >
                ...
                <span
                  style={{
                    textAlign: 'center',
                    fontWeight: 600,
                    color: doc.errorCount > 0 ? '#DC2626' : '#CBD5E1',
                  }}
                >
                  {doc.errorCount}
                </span>
                <span
                  style={{
                    textAlign: 'center',
                    fontWeight: 600,
                    color: doc.warningCount > 0 ? '#D97706' : '#CBD5E1',
                  }}
                >
                  {doc.warningCount}
                </span>
                <span
                  style={{
                    textAlign: 'center',
                    fontWeight: 600,
                    color: doc.infoCount > 0 ? '#3B82F6' : '#CBD5E1',
                  }}
                >
                  {doc.infoCount}
                </span>
              ...
              {unconnectedRefs.map((item, i) => {
                ...
                return (
                  <div
                    ...
                    onKeyDown={(e) => {
                      if (e.key === 'Enter')
                        handleNavigate(item.sourceDocId, item.sectionId, item.fieldId);
                    }}
                    style={{
                      padding: '8px 12px',
                      backgroundColor: cfg.bg,

...
import type { Project, Field } from '@specforge/document-schema';
import type {
  DocumentEditorState,
  TableRowValue,
  ...
  const handleExport = useCallback(() => {
    ...
    try {
      if (target === 'project') {
        ...
        const result = exportTableToCsv(tableInfo.fieldLabel, tableInfo.columns, tableInfo.rows);
        ...
        >
          <p style={{ margin: 0, fontSize: '0.8rem', color: '#64748B' }}>
            ...
            {target === 'document' && selectedDocument && (
              <>
                ドキュメント「{selectedDocument.title}」を {format.toUpperCase()}{' '}
                形式でエクスポートします。
              </>
            )}
            {target === 'table' && <>選択したテーブルを CSV 形式でエクスポートします。</>}
          </p>
        </div>
        ...
        <div style={{ display: 'flex', gap: '12px', justifyContent: 'flex-end' }}>
          ...
          <button
            ...
            style={{
              ...
              color: '#FFFFFF',
              backgroundColor:
                isExporting || (target === 'table' && !selectedTableId) ? '#94A3B8' : '#3B82F6',
              border: 'none',
              borderRadius: '6px',
              cursor:
                isExporting || (target === 'table' && !selectedTableId) ? 'not-allowed' : 'pointer',
            }}
          >

...
function QualityScoreDisplay({
  ...
  const projectErrorCount = projectValidation.issues.filter(
    (issue) => issue.severity === 'error'
  ).length;
  const projectWarningCount = projectValidation.issues.filter(
  ...
  return (
    ...
    >
      ...
      <div style={{ marginTop: '8px', fontSize: '0.72rem', color: '#475569' }}>
        Project: Error {projectErrorCount} / Warning {projectWarningCount}
      </div>
    </div>
  );
}
    ...
    >
      ...
      <nav style={tabBarStyle}>
        {tabs.map((tab) => (
          ...
          >
            {tab.label}
            {tab.id === 'validation' && allValidationItems.length > 0 && (
              <span
                style={{
                  marginLeft: '4px',
                  fontSize: '0.65rem',
      ...
      {activeTab === 'relationship' && (
        <RelationshipPanel
          ...
          onNavigateToDocument={
            onNavigateToField
              ? (docId, secId, fId) => onNavigateToField(docId, secId, fId)
              : undefined
          }
        />
      )}

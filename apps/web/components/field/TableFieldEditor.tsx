  ...
  return (
    <th style={thStyle}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '3px', whiteSpace: 'nowrap' }}>
        ...
        {column.required && (
          <span style={{ color: '#EF4444', fontSize: '0.7rem', fontWeight: 600 }}>*</span>
        )}
      </div>
      {desc && (
        <div
...
export function TableFieldEditor({
  ...
  const handleCellChange = useCallback(
    (rowIndex: number, columnKey: string, value: TableRowValue[string]) => {
  ...
  return (
    ...
    >
        ...
        <table style={tableStyle}>
          <thead style={{ position: shouldVirtualize ? 'sticky' : undefined, top: 0, zIndex: 1 }}>
            <tr>
              ...
              <th style={{ ...thStyle, width: '50px', textAlign: 'center' }}>操作</th>
            </tr>
          </thead>
          <tbody>
            {visibleRows.map((row, rowIndex) => (
              <tr
                key={rowIndex}
                style={{
                  backgroundColor: isRowEmpty(row, columns) ? '#FFFBEB' : undefined,
                  height: shouldVirtualize ? DEFAULT_ROW_HEIGHT : undefined,
                }}
              >
                {columns.map((col) => {
      ...
      <div
        style={{ padding: '8px 10px', borderTop: '1px solid #F1F5F9', backgroundColor: '#FAFBFC' }}
      >
        <button
          type="button"
          style={{

      ...
      <table style={tableStyle}>
        ...
        <tbody>
          {rows.map((row, rowIndex) => (
            <tr
              key={rowIndex}
              style={{ backgroundColor: rowIndex % 2 === 1 ? '#FAFBFC' : 'transparent' }}
            >
              <td
                style={{ ...tdStyle, textAlign: 'center', color: '#94A3B8', fontSize: '0.72rem' }}
              >
                {rowIndex + 1}
              </td>
              {columns.map((col) => {
...
function PreviewField({ field, value }: { field: Field; value: FieldValue }) {
  ...
  return (
    <div style={fieldRowStyle}>
      ...
      <div style={isEmpty ? emptyValueStyle : fieldValueStyle}>{isEmpty ? '—' : displayValue}</div>
    </div>
  );
}
...
function PreviewSection({
  ...
  return (
    <div id={`section-${section.id}`} style={sectionStyle}>
      ...
      {section.description && <p style={sectionDescStyle}>{section.description}</p>}

      {/* Normal fields in label-value rows */}
      {normalFields.length > 0 && (
        <div style={{ marginBottom: tableFields.length > 0 ? '16px' : '0' }}>
          {normalFields.map((field) => (
            <PreviewField key={field.id} field={field} value={fieldValues[field.id]} />
          ))}
        </div>
      )}

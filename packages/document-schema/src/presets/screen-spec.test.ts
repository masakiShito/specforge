...
describe("screenSpecPreset", () => {
  ...
  describe("screen-fields section", () => {
    it("should have table field with correct columns", () => {
      const screenFieldsSection = screenSpecPreset.sections.find(
        (s) => s.key === "screen-fields"
      );
      expect(screenFieldsSection).toBeDefined();

      const tableField = screenFieldsSection?.fields.find(
        (f) => f.valueType === "table"
      );
    ...
    it("should have inputType enum with valid options", () => {
      ...
      const inputTypeColumn = tableField?.table?.columns.find(
        (c) => c.key === "inputType"
      );
  ...
  describe("api-connections section", () => {
    it("should have reference field for API documents", () => {
      ...
      const tableField = apiConnectionsSection?.fields.find(
        (f) => f.valueType === "table"
      );
      const apiRefColumn = tableField?.table?.columns.find(
        (c) => c.key === "apiRef"
      );

      expect(apiRefColumn?.valueType).toBe("reference");
      expect(apiRefColumn?.reference?.kind).toBe("document");
      expect(apiRefColumn?.reference?.constraint?.documentKinds).toContain(
        "api-spec"
      );
    });
  });

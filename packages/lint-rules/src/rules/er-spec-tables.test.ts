...
describe("er spec table validators", () => {
  it("validates entity uniqueness and physical name format", () => {
    ...
    expect(issues.some((issue) => issue.columnKey === "physicalName" && issue.severity === "warning")).toBe(true);
  });
  ...
  it("reports self-referential relationships as info", () => {
    ...
    expect(issues.some((issue) => issue.columnKey === "childEntity" && issue.severity === "info")).toBe(true);
  });
});

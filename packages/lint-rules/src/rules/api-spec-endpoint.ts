      ...
      if (section.key === 'endpoint-basic') {
        if (field.key === 'endpoint' && (!value || (typeof value === 'string' && !value.trim()))) {
          issues.push({
            ...
            reason:
              'APIのエンドポイントは実装に必須の情報です。未入力のままだとAPI仕様書として成立しません。',
            fix: '「エンドポイント」にAPIのURLパス（例: /api/v1/orders）を入力してください。',
          });
        }
        if (
          field.key === 'httpMethod' &&
          ...
          issues.push({
            ...
            reason:
              'HTTPメソッドはAPI定義の基本情報です。未選択だとリクエストの送信方法が不明になります。',
        ...
        if (field.key === 'authRequired' && typeof value !== 'boolean') {
          issues.push({
            ...
            message: '認証要否が未選択です',
            reason:
              '認証の要否が曖昧だと、画面側・API側でセキュリティ前提が一致しないリスクがあります。',

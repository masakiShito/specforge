...
function validateEditorReferenceIntegrity(state: DocumentEditorState, project: Project): DesignValidationIssue[] {
  const issues: DesignValidationIssue[] = [];
  ...
  for (const section of state.document.sections) {
    ...
    for (const field of section.fields) {
      ...
      const rowsValue = Array.isArray(state.fieldValues[field.id]) ? (state.fieldValues[field.id] as TableRowValue[]) : [];
  ...
  for (const section of state.document.sections) {
    ...
    for (const field of section.fields) {
      ...
      const hasAnyApiRef = apiConnectionRows.some((row) => getCellReferenceDocumentId(row, "apiRef") !== "");
      ...
      rowsValue.forEach((row, rowIndex) => {
        ...
        const isApiAction = API_ACTION_KEYWORDS.some((keyword) => actionType.toLowerCase().includes(keyword.toLowerCase()));
        ...
        if (isApiAction && !hasAnyApiRef && project.documents.some((document) => document.kind === "api-spec")) {
...
export function extractReferences(value: TableRowCellValue | TableRowValue): LegacyReferenceValue[] {
    ...
    Object.entries(doc.sections).forEach(([sectionKey, section]) => {
      Object.entries(section.fields).forEach(([fieldKey, field]) => {
        if (field.type === "table" && field.rows) {
          issues.push(...validateReferenceIntegrity(documentId, sectionKey, fieldKey, field.rows, documentMap));
        }
      });
    });
  });

...
describe("business rule table validators", () => {
  it("validates rule uniqueness and condition/action consistency", () => {
    ...
    expect(issues.some((issue) => issue.columnKey === "action" && issue.severity === "warning")).toBe(true);
    expect(issues.some((issue) => issue.columnKey === "condition" && issue.severity === "warning")).toBe(true);
  });
  ...
  it("recommends validation error messages", () => {
    ...
    expect(issues.some((issue) => issue.columnKey === "errorMessage" && issue.severity === "info")).toBe(true);
  });
});

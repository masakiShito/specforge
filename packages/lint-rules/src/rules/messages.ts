...
export function validateMessages(
  ...
  rows.forEach((row, rowIndex) => {
    ...
    if (messageType === 'confirm' && messageText) {
      const looksLikeConfirm = CONFIRM_KEYWORDS.some((kw) => messageText.includes(kw));
      if (!looksLikeConfirm) {
        issues.push({
          ...
          reason:
            'confirm タイプのメッセージは、ユーザーに確認を促す文言（例：「よろしいですか？」）を含むのが一般的です。',
  ...
  if (errorMsgsWithoutCondition >= 3) {
    issues.push({
      ...
      reason:
        'error メッセージは特定の条件下で表示されるのが一般的です。表示条件がないと、いつ表示されるか実装者が判断できません。',
